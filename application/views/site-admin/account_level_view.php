...
<?php echo form_open( current_url().'/process_bulk' ); ?>
	...
	<table class="list_item" id="sortable">
		...
		<tbody>
			...
			<tr class="state-default<?php if ( $row->level_group_id == '1' || $row->level_group_id == '2' || $row->level_group_id == '3' ): ?> ui-state-disabled<?php endif; ?>" id="listItem_<?php echo $row->level_group_id; ?>">
				<td><input type="checkbox" name="id[]" value="<?php echo $row->level_group_id; ?>"<?php if ( $row->level_group_id == '1' || $row->level_group_id == '2' || $row->level_group_id == '3' ): ?> disabled="disabled"<?php endif; ?> /></td>
				<td class="cursor_drag_ns"><?php echo $row->level_priority; ?></td>
				<td><?php echo $row->level_name; ?></td>
				<td><?php echo $row->level_description; ?></td>
				<td><?php echo anchor( current_url().'/edit?id='.$row->level_group_id, lang( 'admin_edit' ) ); ?></td>
			</tr>
			<?php endforeach; ?>
			<?php else: ?>
			<tr>
