...
<div class="list_item_cmdleft">
	...
	<?php echo sprintf( lang( 'account_total' ), ( isset( $list_item['total_account'] ) ? $list_item['total_account'] : '0' ) ); ?>
</div><!--.list_item_cmdleft-->
<div class="list_item_cmdright">
	<form method="get" action="">
		<input type="text" name="q" value="<?php echo htmlspecialchars( $this->input->get( 'q', true ), ENT_QUOTES, 'UTF-8' ); ?>" />
...
<?php echo form_open( current_url().'/process_bulk' ); ?>
	...
	<table class="list_item">
		...
		<tbody>
			...
			<tr>
				<td><?php echo form_checkbox( 'id[]', $row->account_id); ?></td>
				<td><?php echo $row->account_id; ?> [<?php echo $this->account_model->show_account_level_info( $row->account_id, true ); ?>]</td>
				<td><a title="<?php echo $row->account_fullname; ?>" class="cursor_default"><?php echo $row->account_username; ?></a></td>
				<td><?php echo $row->account_email; ?></td>
				<td><?php echo $row->account_create; ?></td>
				<td><?php echo $row->account_last_login; ?></td>
				<td>
					<img src="<?php echo base_url(); ?>client/images/<?php echo ( $row->account_status == '1' ? 'yes' : 'no' ); ?>.gif" alt="<?php echo lang( 'account_status' ); ?>" />
					<?php echo ( $row->account_status == '0' ? $row->account_status_text : '' ); ?>
				</td>
				<td>
					<?php echo anchor( $this->uri->segment(1).'/'.$this->uri->segment(2).'/edit?id='.$row->account_id, lang( 'admin_edit' ) ); ?>
					<?php echo anchor( $this->uri->segment(1).'/'.$this->uri->segment(2).'/viewlog?id='.$row->account_id, lang( 'account_view_logins' ) ); ?>
				</td>
			</tr>
			<?php endforeach; ?>
			<?php else: ?>
