...
class profile extends MX_Controller {
	...
	function index() {
		...
		if ( $query->num_rows() > 0 ) {
			...
			$output['signature'] = htmlspecialchars_decode( $row->account_signature, ENT_QUOTES );
		...
		if ( $_POST ) {
			...
			// +avatar
			$data['account_signature'] = trim( htmlspecialchars( $this->input->post( 'signature', true ), ENT_QUOTES, 'UTF-8' ) );
			...
			// +avatar
			$output['signature'] = htmlspecialchars_decode( $data['account_signature'], ENT_QUOTES );
		}
		// headr tags output###########################################
		...
		// output
		$this->load->view( 'account/edit_profile_view', $output );
	}// index
