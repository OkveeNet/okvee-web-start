...
class account_permission extends admin_controller {
	...
	function getnew() {
		// check permission
		if ( $this->account_model->check_admin_permission( 'account_permissions', 'account_manage_permission' ) == false ) {redirect( 'site-admin' );}
		$permission_page = trim( strip_tags( $this->input->get( 'perm_page' ) ) );
		$this->modules_model->getnew( $permission_page );
	...
	function remove_one() {
		...
		$this->db->delete( 'account_level_permission' );
		echo "<script type=\"text/javascript\">history.go(-1);</script>\n";
	}// renove_one
		...
		if ( $this->input->post() ) {
			...
			if ( is_array( $list_permissions ) ) {
				foreach ( $list_permissions as $key => $item ) {
					...
					$this->db->update( 'account_level_permission' );
