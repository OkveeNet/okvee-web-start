...
class account extends admin_controller {
	...
	function add() {
		// check permission
		if ( $this->account_model->check_admin_permission( 'account_account', 'account_add' ) == false ) {redirect( 'site-admin' );}
		...
		if ( $_POST ) {
			...
			$data['account_signature'] = trim( htmlspecialchars( $this->input->post( 'account_signature', true ), ENT_QUOTES, 'UTF-8' ) );
			...
			$output['account_signature'] = htmlspecialchars_decode( $data['account_signature'], ENT_QUOTES );
	...
	function deletelog() {
		// check permission
		if ( $this->account_model->check_admin_permission( 'account_account', 'account_view_logins' ) == false ) {redirect( 'site-admin' );}
		// get id
		...
		} elseif ( $cmd == 'truncate' && $this->account_model->show_account_level_info() === '1' ) {
	...
	function edit() {
		// check permission
		if ( $this->account_model->check_admin_permission( 'account_account', 'account_edit' ) == false ) {redirect( 'site-admin' );}
		// get id
		$id = trim( $this->input->get( 'id', true ) );
		if ( $id == null ) {
			$ca_account = $this->account_model->get_account_cookie( 'admin' );
		...
		if ( $query->num_rows() > 0 ) {
			...
			$output['account_signature'] = htmlspecialchars_decode( $row->account_signature, ENT_QUOTES );
	...
	function index() {
		// check permission
		if ( $this->account_model->check_admin_permission( 'account_account', 'account_manage' ) == false ) {redirect( 'site-admin' );}
	...
	function process_bulk() {
		// check permission
		if ( $this->account_model->check_admin_permission( 'account_account', 'account_delete' ) == false ) {redirect( 'site-admin' );}
	...
	function viewlog() {
		...
		$output['list_item'] = $this->account_model->list_account_logins( $id );
		if ( is_array( $output['list_item'] ) ) {
			$output['pagination'] = $this->pagination->create_links();
		}
