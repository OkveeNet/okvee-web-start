...
class account_level extends admin_controller {
	...
	function add() {
		// check permission
		if ( $this->account_model->check_admin_permission( 'account_level', 'account_add_level' ) == false ) {redirect( 'site-admin' );}
		$output = '';
		if ( $_POST ) {
	...
	function ajax_sort() {
		// check permission
		if ( $this->account_model->check_admin_permission( 'account_level', 'account_manage_level' ) == false ) {redirect( 'site-admin' );}
	...
	function edit() {
		// check permission
		if ( $this->account_model->check_admin_permission( 'account_level', 'account_edit_level' ) == false ) {redirect( 'site-admin' );}
	...
	function index() {
		// check permission
		if ( $this->account_model->check_admin_permission( 'account_level', 'account_manage_level' ) == false ) {redirect( 'site-admin' );}
		$output['list_item'] = $this->account_model->list_level_group();
	...
	function process_bulk() {
		// check permission
		if ( $this->account_model->check_admin_permission( 'account_level', 'account_delete_level' ) == false ) {redirect( 'site-admin' );}
				...
				if ( $an_id != '1' && $an_id != '2' && $an_id != '3' ) {
					if ( $cmd == 'del' ) {
						...
						if ( $this->account_model->can_i_add_edit_account( $an_id ) == true ) {
							...
							// delete
							$this->db->where( 'level_group_id', $an_id );
							$this->db->delete( 'account_level_group' );
						}
					}
				}
			}
